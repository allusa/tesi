...
totals. Així, en aquests casos, no hi ha una interpretació clara de
què hi ha entre dues mesures.  A continuació ho exemplifiquem amb una
representació que anomenem parcial discreta.

\todo{pensar un nom que no sigui $d$}

\begin{definition}[Funció de representació parcial discreta]
  Sigui $S=\{m_0,\ldots,m_k\}$ una sèrie temporal, es defineix
  $\glsdispdef{not:discreta}{S^{\gls{pd}}(t)}$ com la funció de representació
  discreta de la sèrie temporal $\forall m \in S: S^{\gls{pd}}(t) =
  \begin{cases}
    V(m) & \text{si }  t=T(m) \\
...
\begin{example}[Sèrie temporal amb representació discreta]
  Sigui la sèrie temporal $S=\{ (3,1), (4,3), (6,2), (9,1) \}$, el
  graf de la representació discreta és $\glssymbol{not:sgst:graf}
...
les funcions d'impulsos (\emph{impulse train function}) però que
pertany a les funcions definides a trossos.  A continuació ho
exemplifiquem amb una representació que anomenem delta ($\delta$)
perquè es basa en la funció delta de Dirac, la qual val zero a tot
arreu excepte en el punt zero.

\begin{definition}[Funció de representació delta]
  Sigui $S=\{m_0,\ldots,m_k\}$ una sèrie temporal i $T$ el domini del
  temps, es defineix $\glsdispdef{not:dd}{S^\delta(t)}$ com la
