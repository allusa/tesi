...
representació \emph{first-order hold} (\acro{FOH}), el qual consisteix
en interpolar linealment cada valor fins al proper.


La definició del \acro{FOH} amb funcions matemàtiques contínues es construeix
...
\[
S_ R^\text{\acro{FOH}}(t) = \sum_{t\in T} V(m)
...
\begin{definition}[Funció de representació \emph{first-order hold}]
  Sigui $S=\{m_0,\ldots,m_k\}$ una sèrie temporal i $T$ el domini del
  temps, es defineix $S^\text{\acro{FOH}}(t)$ com la funció de representació
...
\begin{example}[Sèrie temporal amb representació \acro{FOH}]
  Sigui la sèrie temporal $S=\{ (3,1), (4,3), (6,2), (9,1) \}$, la
  seva representació \acro{FOH} és $S^\text{\acro{FOH}}(t) =
  ...
  S^\text{\acro{FOH}}(t)$, es mostra a la \autoref{fig:model:repr:foh}
  ...
  \begin{tikzpicture}[baseline=(current bounding box.center)]
   ...
   \end{tikzpicture}
   \caption{Taula d'una sèrie temporal $S$ i
     $\glssymbol{not:sgst:graf} S^\text{\acro{FOH}}(t)$}
...
basen en interpolacions que passin exactament pels punts donats.  
...
\todo{nova def} Aplicant la pseudoinversa, es pot generalitzar el problema a
una regressió n-polinomial $S^{\text{polinomi}}(t)=\alpha_0 + \alpha_1 t + \dotsb +\alpha_n t^n$ amb $B$ la mateixa matriu que abans i 
\[
