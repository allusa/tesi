...
\begin{definition}[Temps]
  ...
  Sigui $T$ el domini del temps, definim un instant de temps $t^i$ com un
  element del conjunt $T$. \todo{...}
  
  Sigui $t^d$ una durada i $t^{R}$ un instant de temps marc de
  referència, definim un instant de temps $t^i$ com un valor $t^i
  \in\bar{\R{}}$ que mesura la quantitat de temps respecte al temps de
  referència $t^i= t^{R} + t^d$ . Aquest valor de referència
  $t^{R}\in\R{}$ és també un instant de temps però que permet definir
  unívocament la posició de qualsevol altre instant de temps. \todo{potser quedaria més clar si un instant de temps fos un tuple $(t^i,t^R)$ a on  $t^i= t^{R} + t^d$?}
\end{definition}
