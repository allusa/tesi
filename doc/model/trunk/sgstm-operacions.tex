...
conjunt de valors.
 \begin{definition}[Mapa d'una sèrie temporal multiresolució] %angl. span
   Sigui $M=\{R_0,\dotsc,R_d\}$ una sèrie temporal multiresolució i
   $f$ una funció sobre una subsèrie resolució que retorna un valor
   $v'$ a on $f:R\mapsto v'$, el mapa de $f$ a $M$ és un conjunt de
   valors resultant d'aplicar la funció a cada subsèrie resolució:
   $\glssymboldef{not:sgstm:map}(M,f) = \{ f(R_0), \dotsc, f(R_d) \}$.
\end{definition}
...
temporal multiresolució si la funció és de la forma $f:R\mapsto R'$.
...
\begin{definition}[Canvi de mida d'una subsèrie resolució]
  ...
  s'aplica l'operació $\forall m_i \in S_D: \addD(D',m_i)$ amb
  $D'=(\{\},k')$ com a disc inicial.
\end{definition}
...
Els \glsdispsec{not:sgstm:consulta}{operadors de consulta} en un SGSTM
