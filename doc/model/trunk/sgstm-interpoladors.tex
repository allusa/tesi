...
temporal. En la consolidació dels buffers
(v.~\autoref{sec:model:buffer}) l'interval de temps es correspon a
$t_a=\tau$ i $t_b=\tau+\delta$ i és consolidable quan existeix una
mesura $T(m)\geq\tau+\delta$. A la \autoref{fig:sgstm:agg} dibuixem
les mesures d'una sèrie temporal en vermell, un interval de
consolidació del buffer en línies blaves i la mesura resultant de
consolidació en verd.  Així, sigui
...
calcula la funció d'agregació d'atributs $m'=f(S,[t_a,t_b])$. 
En el dibuix, per simplificar, assumim $T(m')=t_b$. 
De manera general, el càlcul del valor resultant és una agregació sobre:
\begin{itemize}
