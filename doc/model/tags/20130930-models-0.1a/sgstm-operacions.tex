...
\begin{definition}[Afegeix multivalor a una subsèrie resolució]
  ...
  $\text{afegeixMultivalor}(R) = (S_B',S_D',\delta,\tau,k,f)$ amb les
...
\begin{definition}[Unió de dues subsèries resolució]
  ...
  $\text{unioR}(R_1,R_2)$ és una subsèrie resolució $R' =
  (S'_B,S'_D,\delta',\tau',k',f')$ que conté la unió de les sèries
  temporals dels seus buffers i discs: $\text{unioR}(R_1,R_2)=
...
\begin{definition}[Unió de dues sèries temporals  multiresolució]
  ...
  multiresolució, la unió de les dues $\text{unioM}(M_1, M_2)$ és una
  sèrie temporal multiresolució $M'=\{R_0',\dotsc,R_d'\}$ que conté
  les subsèries que no intersequen i la unió de les subsèries que
  intersequen: $\text{unioM}(M_1, M_2)= M_{a} \cup M'_{1} \cup M'_{2}$
  a on $M_a = \{\forall R_1\in M_1,R_2\in M_2: \text{unioR}(R_1,R_2) |
  (\delta_1,f_1) = (\delta_2,f_2) \}$, $M_1' =\{\forall R_1\in M_1: R_1
  | \nexists R_2 \in M_2 : (\delta_1,f_1) = (\delta_2,f_2) \}$ i $M_2'
  =\{\forall R_2\in M_2: R_2 | \nexists R_1 \in M_1 : (\delta_1,f_1) =
  (\delta_2,f_2) \}$.
\end{definition}
...
\begin{definition}[Junció de dues subsèries resolució]
  ...
  junció de les dues subsèries resolució $\text{juncioR}^r(R_1,R_2)$ és
  una subsèrie resolució $R' = (S'_B,S'_D,\delta',\tau',k',f')$ que
  conté la junció temporal de les sèries temporals dels seus buffers i
  discs: $\text{juncioR}^r(R_1,R_2)=
  (S'_B,S'_D,\delta_1,\max(\tau_1,\tau_2),k_1+k_2,f_1)$ a on $S'_B =
...
\begin{definition}[Junció de dues sèries temporals multiresolució]
  ...
  les dues $\text{juncioM}^r(M_1, M_2)$ és una sèrie temporal
  multiresolució $M'=\{R_0',\dotsc,R_d'\}$ que conté les subsèries que
  no intersequen ampliades amb un multivalor i la junció de les
  subsèries que intersequen: $\text{juncioM}^r(M_1, M_2)= M_{a} \cup
  M'_{1} \cup M'_{2}$ a on $M_a = \{\forall R_1\in M_1,R_2\in M_2:
  \text{juncioR}(R_1,R_2) | (\delta_1,f_1) = (\delta_2,f_2) \}$, $M_1'
  = \{\forall R_1\in M_1: \text{afegeixMultivalor}(R_1)| \nexists R_2
  \in M_2 : (\delta_1,f_1) = (\delta_2,f_2) \}$ i $M_2' = \{\forall
  R_2\in M_2: \text{afegeixMultivalor}(R_2)| \nexists R_1 \in M_1 :
...
Així doncs, els operadors de consulta en un SGSTM en primer lloc es
basen en obtenir sèries temporals de la base de dades multiresolució
per a en segon lloc aplicar-hi les operacions dels SGST.  En aquest
apartat definim el operadors que permeten obtenir les sèries temporals
a partir d'una sèrie temporal multiresolució; l'aplicació posterior
d'operadors a les sèries temporals segueix les mateixes
definicions que en el model d'operacions dels SGST
...
\begin{definition}[Selecció de la sèrie temporal d'un disc]
  ...
  $\text{SerieDisc}(M,\delta,f)= S'$ a on $S'= S_D' |
...
\begin{definition}[Sèrie temporal total]
  ...
  temporal multiresolució $\text{SerieTotal}(M^*)$ és la sèrie
  temporal que resulta de la concatenació de les sèries temporals dels
  discs per ordre de pas de consolidació: $\text{SerieTotal}(M^*)=S'$
  a on $S'= S_{D0} || S_{D1} || S_{D2} || \dotsb || S_{Dd}$
  complint que $\forall (S_{Bi},S_{Di},\delta_i,\tau_i,k_i,f_i) \in
  M^* : \delta_0 < \delta_1 < \delta_2 < \dots < \delta_d$.
\end{definition}
...
\item Per a extreure una resolució determinada de la sèrie temporal
  ...
  $\text{SerieTotal}(M)[i]^r$ a on $i$ és un conjunt d'instants de
  temps i $r$ la representació de la sèrie temporal.
...
\item Per a sumar dues sèries temporals emmagatzemades a una base de
  ...
  $\text{SerieTotal}(M_1)+\text{SerieTotal}(M_2)$.
