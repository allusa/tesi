...
$[t_b,t_b+\delta]$, i a l'etapa reduce ja es calcula correctament
$f(S,[t_b-\delta,t_b])$. \todo{això ho veurem en un exemple a continuació}




En resum, el model de programació limita les capacitats dels
...
\todo{ALERTA!!}
Per això hi ha el paràmetre -mapg
Hem de fer la selecció $S(t_b-\delta,t_b+\delta]$,
Ara haurem d'aplicar el problema de les $f$ per a \gls{zohe}. És a dir que hem de classificar(m,e) en $t_b$ i $t_b-\delta$
...
l'stdout que es mostren al~\autoref{lst:rrdoop:sortidashell}.  En el
procés de reduce cal tenir en compte que hi pot haver més d'un
identificador per processar, com és el cas de l'execució en la shell. 


9 mesures consolidades finals


Les dades de sortida també són un fitxer o fitxers de dades\todo{}

S'observa que RoundRobindoop no té en compte les $k$: apareixen el 5/mean_zohe-5 que ja hauria d'haver estat eliminat i el 5/mean_zohe-30 o el 10/maximum_zohe-30  que encara no es podien calcular: de fet tenen valor nul (None)
