...
les d'entremig i les finals, i finalment quina operació realitzen els
map i els reduce.
...
\begin{figure}[tp]
 ...
 \draw[myarrow] (d1b.center) -- (r2) node[sloped,above,near end] {$E_{0}^1$};

 \draw[myarrow] (d2.center) -- (r1) node[sloped,above,near end] {$E_{1}^0$};
 \node (d2b) [above=3ex of d2.280] {}; 
 \draw[myarrow] (d2b.center) -- (r2) node[sloped,above,near end] {$E_{1}^1$};

 \draw[myarrow,dotted] (dn) -- (r1) node[sloped,above,near end] {$E_{d}^0$};
 \draw[myarrow,dotted] (dn) -- (r2) node[sloped,above,near end] {$E_{d}^1$};
...
de dades pendents de consolidar $E=\{ D_{0}, \dotsc, D_t\}$ \todo{s'ha
de dir quan val $t$} on cada dada pendent de consolidar és un tuple
...
\todo{ERROR:cada reduce calcula amb les mesures de delta,f i n} Les
...
\todo{en les defincions, escriure completament cada variable de partida}
...
\begin{definition}[Operació Reduce]
  ...
  d'atributs de la definició \todo{ref a la secció faa}.
\end{definition}
