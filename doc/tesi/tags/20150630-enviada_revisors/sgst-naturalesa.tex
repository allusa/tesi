...
%
% Cal aclarir que en el model
% hem classificat les operacions de les sèries temporals segons
% naturalesa de conjunt, de seqüència i de funció temporal. Aquesta classificació, però,
% els trets semàntics no es corresponen amb l'estudi de
% les sèries temporals com a conjunt, com a seqüència ni com a funció
% temporal que hem dut a terme en la formalització matemàtica del model
% d'operacions.
...
\textcite[cap.~1]{proakismanolakis96} els senyal discrets tenen
principalment dos orígens: l'adquisició mitjançant el mostreig d'un
senyal continu cada cert període de temps o l'adquisició que prové
d'una acumulació o d'un comptatge d'esdeveniments en un cert període
de temps.
%
...
\autoref{fig:model:magnitud} es mostra una sèrie temporal
...
aquestes formes d'informació mitjançant tres sèries temporals
d'exemple provinents del mateix comptatge:
...
\item $S_v = \{(0,0),(2,0{,}5),(4,1),(5,1),(8,2)\}$ amb els valors de
  ...
  (v.\ \autoref{ex:sgst:duplicat} i \autoref{ex:sgst:increments}) i les unitats dels valors són de \emph{unitat$/$unitat de temps}.
\end{itemize}
...
\begin{example}[Sèrie temporal amb trets de magnitud i de comptador]
...
com a conjunt dens són els reals, $\glssymbol{not:temps-domini} =
\glssymbol{not:Rb}$, i com a conjunt discret són els naturals,
$\glssymbol{not:temps-domini} = \glssymbol{not:N}$.
...
\begin{example}[Sèrie temporal amb tret de seqüència densa]
  ...
  $\glssymbol{not:temps-domini} =\{\text{gen99},\text{feb99},\text{març99},\dotsc\}$ i cada
...
\begin{example}[Sèrie temporal amb tret de seqüència discreta]
  ...
  $\glssymbol{not:temps-domini}=\{\text{oct98},\text{oct99},\text{oct00},\dotsc\}$
  i cada instant de temps es pot observar com un lapse que dura tot un
  mes d'octubre però no hi ha continuïtat del temps entre els
  mesos. Així doncs, aquestes sèries temporals presenten més
  dificultats a l'hora d'expressar-se amb temps dens, per exemple com
  en el cas anterior es pot precisar amb els instants de temps
...
\begin{example}[Sèrie temporal amb tret de seqüència agregada]
  ...
  aquest cas el temps és discret amb el domini
  $\glssymbol{not:temps-domini}=\{\text{gen},\text{feb},\dotsc,\text{des}\}$
  i el lapse dura tot un mes però sense cap posició absoluta en el
  sistema de coordenades de temps. Aquest cas no es pot expressar amb
  temps dens.  
  
  Encara que aquest i casos semblants compleixen les
  propietats de sèries temporals, la interpretació del temps és
  dificultosa i pot resultar més còmode tractar-los com a casos
  genèrics dels \gls{SGBDR} on la interpretació de l'atribut de temps
  com a característica agrupada serà més apropiada.
\end{example}
...
\begin{example}[Intervals temporals del tipus d'interès]
  \label{ex:sgst:bce}
...
\begin{example}[Intervals temporals d'una agenda]
...
%  El
% model de \gls{SGST} definit formalitza la sèrie temporal com una
% representació discreta.
Una sèrie temporal pot representar discretament una funció temporal
(original), és a dir una funció que depèn de la variable temps. Una
sèrie temporal pot representar-se com una funció temporal, és a dir
obtenir nous valors a partir de la sèrie temporal segons una
funció. Aquesta nova co\l.lecció de valors s'anomena graf d'una funció
(\emph{graph of a function}), en el sentit de gràfic (\emph{plot}) que
cal no confondre amb els grafs d'arestes i vèrtexs (\emph{vertex-edge
  graph}).  Així, una sèrie temporal pot representar-se com una funció
que descrigui o s'aproximi a com era la funció temporal original
representada. En aquesta secció, però, descrivim com poden
representar-se independentment que s'ajustin a la funció temporal
original.
...
\begin{definition}[Funció de representació]
  ...
  Sigui $S$ una sèrie temporal, es defineix $S(t)$ com la funció de
  representació de la sèrie temporal contínua al llarg del temps, $t\in
  \glssymbol{not:temps-domini}$. És a dir que per cada instant de
  temps la funció pren un valor i per tant és una funció
  $\glssymbol{not:temps-domini}\rightarrow\glssymbol{not:valor-domini}$.
  ...
  superíndex \glsdispdef{not:sgst:frepr}{$S^r(t)$} on
...
\begin{definition}[Graf d'una sèrie temporal]%an. graph
  Sigui $S(t)$ la funció de representació d'una sèrie temporal i
  $\glssymbol{not:temps-domini}$ un domini del temps, es defineix el
  graf de la sèrie temporal, $\glssymboldef{not:sgst:graf} S(t)$, com
  un conjunt de parells ordenats $(t,S(t))$ de manera que
  $\glssymboldef{not:sgst:graf} S(t) = \{ (t,S(t)) |
  t\in\glssymbol{not:temps-domini} \}$.
\end{definition}
...
temporals. En \textref{sec:art:seriestemporals} s'ha comentat
...
\begin{definition}[Funció de representació parcial discreta]
  Sigui $S$ una sèrie temporal, es defineix
...
sèrie temporal. Sigui $S=\{m_0,\ldots,m_k\}$ una sèrie temporal
aleshores $\glssymbol{not:sgst:graf} S^{\gls{pd}}(t) \equiv
\{m_0,\ldots,m_k\}$.
...
pertanyen a les funcions definides contínues a trossos.  A continuació
...
\begin{definition}[Funció de representació delta de Dirac]
  Sigui $S$ una sèrie temporal i $\glssymbol{not:temps-domini}$ el
  domini del temps, es defineix $\glsdispdef{not:dd}{S^{\gls{dd}}(t)}$
  com la funció de representació delta de Dirac al llarg del temps,
  $\forall m \in S:$
  \begin{align*}
    S^{\gls{dd}}(t) = &  \\
    = & \sum_{t\in\glssymbol{not:temps-domini} } V(m) \delta(t-T(m)): \delta(t)= 
      \begin{cases}
        1 & \text{si }  t=0 \\
        0 & \text{altrament}
      \end{cases} \\
...
funcions definides constants a trossos \emph{piecewise constant
...
\begin{definition}[Funció de representació \emph{zero-order hold}]
  \label{def:sgst:zoh}
  Sigui $S$ una sèrie temporal i $\glssymbol{not:temps-domini}$ el domini del
  temps, es defineix $S^{\gls{zoh}}(t)$ com la funció de representació
  \emph{zero-order hold} al llarg del temps, $\forall m \in S:$
  \begin{align*}
    S^{\gls{zoh}}(t) = &  \\
    = & \sum_{t\in \glssymbol{not:temps-domini}} V(m) \glssymbol{not:Ia}_{\big[T(m), T(\glssymbol{not:sgst:next}(m)) \big)}(t)\\
    = & \begin{cases}
      0 & \text{si }  t<T(\min(S)) \\
      V(m) & \text{si } t\in
...
RRDtool \parencite{lisa98:oetiker}.
\begin{definition}[Funció de representació \emph{zero-order hold} cap
  ...
  \begin{align*}
    S^{\gls{zohe}}(t) = &  \\
    = & \sum_{t\in \glssymbol{not:temps-domini}} V(m) \glssymbol{not:Ia}_{\big(T(\glssymbol{not:sgst:prev}(m)),T(m)\big]}(t) \\
    = & \begin{cases}
      0 & \text{si }  t > T(\max(S)) \\
...
\begin{definition}[Funció de representació \emph{zero-order hold}
  ...
  \begin{align*}
    S^\text{\acro{ZOHC}}(t) = &  \\
   = & \sum_{t\in \glssymbol{not:temps-domini}} V(m) \glssymbol{not:Ia}_{\left[
...
\begin{definition}[Funció de representació rectangular]
  ...
  \begin{align*}
    S^\text{rect}(t) = &  \\
    = & \sum_{t\in \glssymbol{not:temps-domini}} V(m) \operatorname{rect}(t):  \operatorname{rect}(t) = 
    \begin{cases}
...
són les que es basen en funcions definides lineals a trossos
...
triangular general.  Sigui $R$ una sèrie temporal
regular amb període $p$ i sigui $\glssymbol{not:temps-domini}$ el domini del temps, es defineix
$R^\text{FOH}(t)$ com la funció de representació \emph{first-order
  hold} al llarg del temps, $\forall m \in R:$
\[
R^\text{\gls{foh}}(t) = \sum_{t\in \glssymbol{not:temps-domini}} V(m)
...
\begin{definition}[Funció de representació \emph{first-order hold}]
  ...
  \begin{align*}
    S^\text{FOH}(t) = & \\
    = & \sum_{t\in \glssymbol{not:temps-domini}} y_1
    \left(
...
\begin{example}[Sèrie temporal amb representació \gls{foh}]
  ...
  1\operatorname{tri}^{\text{seg}}\left(\frac{t-9}{+\infty-9}\right)
  \end{align*}
...
per mínims quadràtics a les mesures de la sèrie temporal. L'anomenem
...
\begin{definition}[Funció de representació lineal]
  ...
  $S^\text{lineal}(t)$ com la funció de representació de regressió
  lineal al llarg del temps, $\forall m \in S: S^\text{lineal}(t) =
  \alpha + \beta t$ on els valors de la regressió lineal es calculen
  aplicant:
  \begin{align*}
    & 
    \left[\begin{array}{c}
...
Aplicant una equació similar es pot generalitzar el problema a
una regressió n-polinomial.
\begin{definition}[Funció de representació polinomial]
  Sigui $S=\{m_0,\ldots,m_k\}$ una sèrie temporal i $n$ el grau del
  polinomi que es vol ajustar, es defineix $S^{\text{polinomi}}(t)$
  com la funció de representació de regressió polinomial al llarg del
  temps, $\forall m \in S: S^{\text{polinomi}}(t)=\alpha_0 + \alpha_1
  t + \dotsb +\alpha_n t^n$ on els valors de la regressió es calculen
  aplicant:
  \begin{align*}
    & 
    \left[\begin{array}{c}
...
A la definició de l'operació d'interval temporal
(v. \autoref{def:sgst:intervalt}) hem deixat oberta la interpretació
del mètode de representació.  A continuació particularitzem l'interval
temporal segons quatre representacions: \gls{dd}, \gls{zohe},
\gls{foh} i lineal.

\begin{definition}[Interval temporal \gls{dd}]
  \label{def:sgst:interval-temporal-delta}
  Sigui $S$ una sèrie temporal, $[s,t]$ un interval de temps i el
  mètode de representació $\gls{dd}$. Definim l'interval temporal
  \gls{dd}, $S[s,t]^{\gls{dd}}$, com la sèrie temporal
  $S[s,t]^{\gls{dd}} = S[s,t] \cup \{(s,0), (t,0)\}$.
\end{definition}
...
\begin{definition}[Interval temporal \gls{zohe}]
  ...
  Sigui $S$ una sèrie temporal, $[s,t]$ un interval de temps i el
  mètode representació $\gls{zohe}$. Definim l'interval temporal
  \gls{zohe}, $S[s,t]^{\glssymbol{not:zohe}}$, com la sèrie temporal
    $S[s,t]^{\glssymbol{not:zohe}} = S(s,t] \cup \{m\}$ on $m=(t,v)$
    i $v= V(\inf( S[t,+\infty] ))$.
\end{definition}
...
\begin{definition}[Interval temporal \gls{foh}]
  ...
  Sigui $S$ una sèrie temporal, $[s,t]$ un interval de temps i el
  mètode de representació $\gls{foh}$. Definim l'interval
  temporal \gls{foh}, $S[s,t]^{\gls{foh}}$, com la sèrie temporal
  $S[s,t]^{\gls{foh}} = S[s,t] \cup \{m, n\}$ on
  $m=(s,S^{\text{FOH}}(s))$ i $n=(t,S^{\gls{foh}}(t))$.
\end{definition}

\begin{definition}[Interval temporal lineal]
  \label{def:sgst:interval-temporal-lineal}
  Sigui $S$ una sèrie temporal, $[s,t]$ un interval de temps i
  el mètode de representació $\text{lineal}$. Definim
  l'interval temporal lineal, $S[s,t]^{\text{lineal}}$, com la sèrie temporal
  $S[s,t]^{\text{lineal}} = \{m,n\}$ on
  $m=(s,S^{\text{lineal}}(s))$ i
  $n=(t,S^{\text{lineal}}(t))$.
\end{definition}
...
interval de temps $I_0=[t,t+d]$, on $t$ és un instant de
temps i $d$ una durada de temps, i els seus intervals múltiples
$I_n=[t+nd\, ,\, t+(n+1)d]$ per
...
d'adquisició de dades aleshores cal notar que, en l'àmbit de teoria
del senyal, aquests intervals de temps $I_n$ s'anomenen intervals de
mostreig, $d$ s'anomena període de mostreig i $t$ s'anomena temps
inicial del mostreig.
...
\begin{definition}[Sèrie temporal regular]
  ...
  Sigui $S=\{m_0,m_1,\dotsc,m_{k-1},m_k\}$ la permutació d'una sèrie
  temporal en què $T(m_0)<T(m_1)<\dotsb < T(m_{k-1})<T(m)$, sigui $t$
  un instant de temps i sigui $d$ una durada de temps. La sèrie
  temporal \glsdispdef{not:sgst:regular}{$S$ és regular} quan
  $d=T(m_1)-T(m_0) = \dotsb = T(m_k)-T(m_{k-1})$ on definim
  $t=T(m_0)$.
\end{definition}

Si una sèrie temporal és regular, l'anomenem sèrie temporal regular de
període $d$ iniciada a $t$. Si $t$ pot ser qualsevol instant de temps
llavors simplement l'anomenem sèrie temporal regular de període $d$.
Si notem l'àmbit de teoria del senyal, aquest període té relació amb
el període de mostreig.
...
\begin{definition}[Sèrie temporal de temps real]
  ...
  Sigui $S$ una sèrie temporal, $t$ un instant de temps, $d$ una
  durada de temps i $D$ una durada que indica termini. La sèrie
  temporal $S$ és de temps real si i només si $D\leq d$ i la
  subsèrie de cada interval amb termini només conté una mesura
  $\forall n\in\{0,\ldots,|S|-1\}: |S[t+nd,t+nd+D)| =
  1$.
...
mostrejada en temps real de període $d$ iniciada a $t$ i
compliment del termini $D$.  Si $D=d$, es pot anomenar que $S$ és
una sèrie temporal de temps real sense termini.

Pel que fa al temps inicial $t$, en el cas de les sèries temporals
de temps reals pot estar situat entre $T(\min(S))-\delta < t \leq
...
regular a excepció de $T(\min(S))=t$, és a dir de la qual les
...
\begin{definition}[Sèrie temporal amb ultramostreig]
  Sigui $S$ una sèrie temporal, $t$ un instant de
  temps i $d$ una durada de temps, els intervals amb
  ultramostreig de $S$ són aquells en què la
  subsèrie corresponent conté més d'una mesura
  $|S[t+nd,t+(n+1)d)|>1$ on $n\in\glssymbol{not:N}$.
\end{definition}
...
\begin{definition}[Sèrie temporal amb inframostreig]
  ...
  $|S[t+nd,t+(n+1)d)|=0$ on $n\in\glssymbol{not:N}$.
\end{definition}
...
\begin{definition}[Sèries temporals regulars entre elles]
  ...
  Siguin $S_1$ i $S_2$ dues sèries temporals,
  \glsdispdef{not:sgst:regulars_entre_elles}{$S_1$ i $S_2$ són
    regulars entre elles} si i només si tenen el mateix vector de
  temps: $\glssymbol{not:sgst:project}_t(S_1) =
  \glssymbol{not:sgst:project}_t(S_2)$. D'aquesta manera també han de
  tenir el mateix cardinal $|S_1|=|S_2|$.
\end{definition}
...
temporal (v.~\autoref{def:sgst:selecciot}) permet regularitzar una
sèrie temporal. Sigui $S$ una sèrie temporal, $t$ i $d$ els paràmetres
de temps desitjats de regularitat i $k\in\glssymbol{not:N}$ una fita
del rang. Una $S$ regular es pot obtenir mitjançant $S[I]$ on $I=
\{t+nd | n\in \glssymbol{not:N} \wedge n \leq k\}$.
...
Sigui $S$ una sèrie temporal de temps real de període $d$ iniciada a
$t$, una operació de funció temporal que regularitza de manera simple
el temps real és la selecció temporal amb representació \gls{zohe},
$S[I]^{\gls{zohe}}$, amb els instants de temps
$I=\{t+nd | n\in\glssymbol{not:N}, t+nd < T(\sup(S))\}$ : .
...
Sigui $S$ una sèrie temporal d'ultramostreig de període $d$ iniciada a
$t$ i sigui $I$ el conjunt desitjat d'instants de temps regulars. La
regularització per agregació mitjana és
$\glssymbol{not:sgst:map}(R,f)$ on $R=\{ (t,0) |
t\in I \}$ i $f(m) = \left(T(m),
  \glssymbol{not:sgst:mitjanav}\big(S[T(m),
  \glssymbol{not:sgst:next}_{R}(T(m))\big)\right)$.
...
$S[s,t]^\text{mitjana}=\{(s,v)\}$  on
$v=\glssymbol{not:sgst:mitjanav}(S[s,t))$. L'operació temporal
...
casos simples anteriors $S[I]^{\gls{zohe}}$.
...
diferent del concepte de termini de temps real descrit anteriorment ja
que el primer és una durada entre mesures i el segon és una durada amb
relació al període regular. \Textcite{rrdtool} anomena
\emph{heartbeat} a aquest temps de termini entre mesures.
