...
d'atributs, $f$. A diferència de les $f$, treballen sobre dues mesures
$m'=\text{flux}(m_f,m,i)$ per a retornar la mesura resultant $m'$, on
$m$ és la nova mesura que s'ha d'incorporar al flux, $m_f$ és el flux
anterior ja processat i $i=[t_a,t_b]$ és l'interval de consolidació de
$f$.  Per exemple, redefinim les funcions d'agregació \gls{dd} màxim i
mitjana per tal que tinguin orientació a flux:

\begin{itemize}
\item $\text{flux-\glssymbol{not:sgstm:maxdd}}: m_f \times m \times i \mapsto
  m'$ on $V(m')=\max(V(m_f),V(m))$ i $T(m')=\frac{t_b+t_a}{2}$. 


\item $\text{flux-\glssymbol{not:sgstm:mitjanadd}}: m_f \times m
...
\text{flux-\glssymbol{not:sgstm:mitjanadd}}((0,0),m_0,i), m'_1 =
\text{flux-\glssymbol{not:sgstm:mitjanadd}}(m'_0,m_1,i), \dotsc, m'_k
= \text{flux-\glssymbol{not:sgstm:mitjanadd}}(m'_{k-1},m_k,i)$ és
...
\item Sigui la \autoref{def:sgstm:addB}, se'n modifica el comportament
  ...
  \text{flux}(m_B,m,[\tau,\tau+\delta])$ i $\text{flux}$ és una
...
\item Sigui la \autoref{def:model:consolidacio-buffer}, se'n modifica
  ...
  d'identitat de $\text{flux}$. Per exemple $m'_B=(0,0)$ per a l'atribut
