...
En una primera part, que inclou aquest \textref{sec:intro}
d'introducció, es presenta el context i els objectius de la
recerca. En el \textref{cap:estat} es descriu l'estat actual de la
recerca i treballs similars.

En una segona part es dissenya i es formula el model.  En el
\textref{sec:model} s'introdueixen els conceptes de model de
...
\gls{SGST} en el \textref{cap:model:sgst} i el model dels \gls{SGSTM}
en el \textref{cap:model:sgstm}.


En una tercera part s'exploren variacions i reflexions sobre el model
presentat. En el \textref{sec:variacions} s'introdueixen les
...
\textref{cap:funciomultiresolucio} es formula la multiresolució com
...
\textref{sec:multiresolucio:dual} es dissenyen sistemes que utilitzin
alhora \gls{SGST} i \gls{SGSTM}.  En el
\textref{sec:multiresolucio:teoriainformacio} es reflexiona sobre el
...
del model. En el \textref{sec:implementacions} s'introdueixen les
diferents implementacions i es comenten les particularitats de cada
una. En el \textref{sec:implementacio:python} es dissenya la
implementació de referència amb llenguatge Python. En el
\textref{sec:implementacio:mapreduce} es dissenya una implementació
amb computació distribuïda i para\l.lela amb la tècnica MapReduce i en
el sistema Hadoop. En el \textref{sec:implementacio:relacional} es
...
\textref{sec:conclusions} es resumeix la dissertació i s'expressen les
conclusions que se'n poden treure, i en el \textref{sec:futur} es
