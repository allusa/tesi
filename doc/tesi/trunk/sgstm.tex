...
\begin{definition}[Forma canònica]
  ...
  cada subsèrie resolució té la forma $R_i =(B_i,D_i),
  B_i=(S_{B_i},\tau_i,\delta_i,\glssymbol{not:sgstm:f}_i),
  D_i=(S_{D_i},\glssymbol{not:sgstm:k}_i)$ té domini de sèrie temporal
  per les sèries temporals $S_{B_i}$ i $S_{D_i}$, de
  ...
  d'agregació d'atributs; \glsdispdef{not:sgstm:canonica}{en la forma
    canònica} s'escriu com $ M = \{S_B, S_D, \tau,
  \delta,\glssymbol{not:sgstm:k}, \glssymbol{not:sgstm:f}\}, \{ \{
  (S_B, S_{B_0}) , (S_D , S_{D_0}) , (\tau, \tau_0), (\delta,
  \delta_0), (\glssymbol{not:sgstm:k}, \glssymbol{not:sgstm:k}_0), (\glssymbol{not:sgstm:f}, \glssymbol{not:sgstm:f}_0) \} , \dotsc, \{ (S_B, S_{B_k}) , (S_D
