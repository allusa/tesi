...
Cal tenir en compte que $T(m) > \tau$ indica que $\tau$ és el temps
...
(\seeref{sec:model:agregador}). Aquestes funcions principalment
...
(\seeref{def:model:zohe}), implica conèixer la mesura següent a un
...
\begin{align*}
  ...
  % & \{ (\delta,f,\tau+(n-G_g)\delta,t,v), \dotsc,
  % (\delta,f,\tau+(n-G_0)\delta,t,v), \\
  % & (\delta,f,\tau+n\delta,t,v), \\
  % &(\delta,f,\tau+(n+L_0)\delta,t,v), \dotsc,
  % (\delta,f,\tau+(n+L_l)\delta,t,v) \\
  % &| (\delta,f,\tau,k) \in \glssymbol{not:esquemaM} \wedge
  % \tau+(n-1)\delta < t \leq \tau+n\delta \wedge n\in\glssymbol{not:Z}
  % \wedge t > \tau \} \\
...
%   (\delta,f,\tau+(n+L_l)\delta,t,v)
%  | \tau+(n-1)\delta < t \leq
%   \tau+n\delta, n\in\glssymbol{not:Z}, t > \tau \}
...
\begin{example}[Classificació d'una mesura per \gls{zohe}]
  ...
  Mentre que a \textref{ex:mapreduce:classifica} es classifica la
  mesura $(25,1)$ als instants $26$ per a $\delta_1$ i $25$ per a
  $\delta_2$, ara s'afegeixen respectivament els instants $24$ i $20$.
  Així, mentre que a \textref{ex:mapreduce:classifica} s'interpretaven
...
Així, en la computació en diferit, sigui $S$ una sèrie temporal i
