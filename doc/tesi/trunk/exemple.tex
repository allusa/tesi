...
Cada sèrie temporals totals es mostra amb el mètode de representació
...
\glssymbol{not:sgstm:maxzohe} (en taronja), ambdues amb el mètode de
representació \gls{zohe}. En gris es mostra la sèrie temporal original
...
s'amplien un interval. En aquest exemple, però, hi ha dades mancants
de més d'un interval i per tant en cal més d'un per a farcir els
forats: RoundRobindoop no els té en canvi RoundRobinson sí.
