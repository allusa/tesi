...
\cite{zhang11}. 
\todo{sense tenir en compte els unnknown i data validation, que en parlarem més avall}
...
\todo{Teresa: afegir interpoladors d'unknown} With reference to data
validation, attribute interpolation functions can cope with this
process. When data has not been captured or has been captured
erroneously, it must be treated as unknown data.
\begin{itemize}
...
unknown. \todo{seguint amb l'exemple dels reals, fem servir el valor imprpopi infinit}.Let unknown value be represented by infinity.

An attribute interpolation functions marking data as unknown is a one giving unknown value as the resulting measure's value, $f: S \times i \mapsto m'$ where $V(m')=\infty$. 

Per exemple definim un limit, usant l'interpolador màxim anterior:
maxlimit2: return max(2,max(S))

Per exemple definim un termini, si les dades estan més espaiades que 2 es marca com a desconeguda sinó què passa?:
termini2: 


An attribute interpolation functions treating unknown data is a one that can calculate a result when there is unknown values in the original time series, $f: S \times i \mapsto m'$ where $\exists m \in S: T(m)=\infty$. From a purely scientific point of view operating with unknown data yields unknown

Per exemple definim el càlcul d'àrea amb desconeguts, 
