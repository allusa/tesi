...
\emph{aggregate} functions and \emph{consolidation} actions. In
...
There can be different attribute aggregate functions in order to
...
\cite{zhang11}.  Furthermore, attribute aggregate functions copes
...
\item When data is erroneously it must be marked as unknown. For
  ...
  reasonable limits.
\end{itemize}
As a consequence, attribute aggregate functions deals with these
...
For example, we can redefine the arithmetic mean$^{d}$ aggregate function
into one capable of treating unknown values by keeping the known mean;
that is, total known mean is spread on the time interval.
arithmetic mean$^{du}$: $S \times i \mapsto m'$ where $m' = \text{arithmetic mean}^{d}(S'',i)$ and $S''= \{m''\in S':V(m'')\neq \infty\}$.
...
In conclusion, a huge amount of attribute aggregate functions can
