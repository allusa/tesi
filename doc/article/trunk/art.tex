...
%wiki: A very large database, or VLDB, is a database that contains an extremely high number of tuples (database rows), or occupies an extremely large physical filesystem storage space. The most common definition of VLDB is a database that occupies more than 1 terabyte or contains several billion rows, although naturally this definition changes over time.

\cite{mylopoulos96}%diverse areas such as CAD, software engineering, real-time process control, corporate repositories and digital libraries require the construction, efficient access and management of large, shared knowledge bases. 


\cite{deligiannakis07}


\cite{ogras06}                      ``time series, which are usu-
ally continuous and unbounded in nature.''                                      ``          Since the data
points arrive sequentially, storing each data point and per-
forming an offline analysis is prohibitive and random access
to the data is not allowed, unlike the traditional approaches.''
