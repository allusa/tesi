...
solidesa, amb \textcite{date:introduction,date06,date:dictionary} com
...
mateix.  Les relacions es defineixen com una parella de capçalera i
cos. El cos és un conjunt de tuples on cada tuple és un conjunt de
parelles atribut i valor. Tots els tuples d'una mateixa relació tenen
els mateixos atributs, així es distingeix entre la capçalera de la
relació --els atributs-- i el cos de la relació --els tuples.

Per exemple una relació és $r_1=(\{\text{nom},\text{edat} \}, \{
\{(\text{nom},\text{a}),(\text{edat},32)\},
...
(\text{nom},\text{edat}), \{ (\text{a},32),(\text{b},23) \})$.  
En la definició estructural del model relacional, els valors sempre
...
capçalera de la relació $r1$ com $\{\text{nom}:
\text{text},\, \text{edat}:\text{enter} \}$.
...
Hi ha, però, dues relacions que tenen un nom específic. Són la relació
amb la capçalera i els tuples buits $\text{table\_dee} = (\{\},\{\})$
i la relació amb la capçalera buida i un tuple buit $\text{table\_dum}
= (\{\},\{\{\}\})$. Aquestes no tenen una representació clara com a
taula.  \todo{repassar noms dee i dum}
...
Pel que fa als operadors, en el cas del model definit en l'àlgebra
relacional estan fortament relacionats amb l'àlgebra de conjunts. Així
hi ha els operadors habituals de conjunts, com per exemple la unió, la
intersecció o el producte; i altres d'específics per a les relacions,
com per exemple la projecció, la selecció, la junció o el reanomena.
