...
concept and does not seek to be either an efficient or a complete database
system. We implement the \acro{TSMS} and \acro{MTSMS} models using
the Python~\cite{python:doc2} programming language.
...
found in~\cite{llusa:roundrobinson}.
...
classes. \emph{Unified Modeling Language}~(\acro{UML}) diagrams are used to
define the class structures.  Operations are implemented as object
methods, which are not shown in \acro{UML} diagrams, due to space
limitations.
...
graphical representation of a time series. For this purpose we use the
