...
database system. We implement the \acro{TSMS} and \acro{MTSMS} models
using the Python~\cite{python:doc2} programming language.

The implementations of the two models, \acro{TSMS} and
\acro{MTSMS}, are organised respectively in two separated Python
libraries: \texttt{Pytsms} and \texttt{RoundRobinson}.
\texttt{RoundRobinson} strongly depends on \texttt{Pytsms} following
the dependency of \acro{MTSMS} on \acro{TSMS}.  The code of this
...
classes. We use \emph{Unified Modeling Language}~(\acro{UML}) diagrams
to define the class structures. We realized the model operations as
object methods that we do not present in \acro{UML} diagrams, due to
space limitations.
...
the relationships among these objects in a \acro{UML} diagram.  A
...
the disc's time series in the \acro{MTSMS} model.
%
...
In the module \texttt{aggregators}, we implemented some default
...
%  LocalWords:  TSMS MTSMS Pytsms RoundRobinson UML TimeSeries Zohe
%  LocalWords:  RegularProp Matplotlib multiresolution subseries
%  LocalWords:  MultiresolutionSeries addResolution consolidable
%  LocalWords:  seriedisc totalseries aggregators
