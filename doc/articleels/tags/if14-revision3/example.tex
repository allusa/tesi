...
\texttt{RoundRobinson} implementations to create a \acro{MTSDB} and to
query it.
...
\emph{Schema}. We design a \acro{MTSDB} schema that stores a
...
subseries after consolidating the \acro{MTSDB}.  Each graphic
corresponds to the possible $\seriedisc$ queries, i.e., every
resolution disc time series from the \acro{MTSDB}. Each title shows
...
In conclusion, this \acro{MTSDB} schema does not store the complete
...
multiresolution offline. However, a \acro{MTSMS} is designed to
