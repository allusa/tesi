...
systems and sensor networks.  This provides opportunities to collect
...
both data values and their timestamps. The sequences of data values
...
some drawbacks when trying to solve 
the challenging issues of time series in the temporal data domain.   
...
%
In spite of this, \emph{RRDtool} is a very specific application and
...
\item It organises the data in an aggregated way and it allows to
  ...
  characteristic is known as \emph{multiresolution}.  Thus, being
  multiresolution the most salient characteristic of our model, we
  name the formalised system as \emph{Multiresolution Time Series
    Data Base Management System} (\acro{MTSMS}).  It is designed to
...
approaches.
%
The multiresolution concept is inspired by a thorough analysis of
...
%
The model allows to favour more recent data over older one. This is 
...
analyse and index massive collections of time series. They argue
...
\end{enumerate}
Note that time coordinates are assumed to be positive for the time instants 
...
original signal at time instant \emph{now} corresponds to values
$\{u,u,2,5,8,u\}$.
...
\item Voluminous data. Monitoring systems capture a huge amount of
  ...
  render more data if it cannot be appreciated in the screen.
...
\item Data time regularisation. To monitor with a non constant
  ...
  reasons for sampling rate variation: either sampling jitters in
