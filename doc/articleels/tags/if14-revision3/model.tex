...
A time series do not contain two measures at the same time. Therefore,
...
We describe how to apply common set operators to time series. We rely
on how the relational model of \acro{DBMS} describes operations based
...
\begin{example}\label{ex:model:s1s2}
  ...
  both time and value attributes, like measure $(4,0)$. The left
  central area contains the measures in $R$ that only share the time
  attribute with a measure in $S$, like measure $(3,1)$. The central
  right area has a symmetrical meaning. The left and right outer areas
  are the remaining measures of $R$ and $S$ respectively.
\end{example}
