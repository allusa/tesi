...
There are two basic time series queries for a \acro{MTSMS}: (i) a
...
% When two multiresolution time
% series do not have the same multiresolution parameters, then $\totalseries(L) + \totalseries(M)$ must be solved using temporal join, as defined in \ref{?}. 
%However we can consider some multiresolution enhancements:
% note that it has $r$ as a parameter.However, there is also $r$ as a
%parameter, so we find these operations a bit cumbersome.

% 1. They share some $\tau$ and $\delta$, then it is possible to compute
% $\seriedisc(L,\delta,f_1) + \seriedisc(M,\delta,f_2)$ where $f_1$ and
% $f_2$ are any attribute aggregate functions, although generally it
% makes more sense to be the same $f_1=f_2$.

% 2. They share some $\delta$. Then one time series must be translated. $\seriedisc(L,\delta,f_1) + \seriedisc(translate(M),\delta,f_2)$ where $translate(M)=$!!!!

% 3. They do not share any $\delta$. Then temporal join must be used in
% the sum $\totalseries(L) + \totalseries(M)$. However, 
% $\totalseries(L) + \totalseries(M)$ contains less data than $L + M$. 
