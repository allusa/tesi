...
measure. The domain for the values can be of any data type. Valid domains
...
series. In the sequel, the domain for values will be denoted by
$\cal{V}$. 
%
...
should not be applied. For example, the addition of values coming from two
...
time series has a supremum even when it is empty.  Regarding the time
domain being the set of affinely extended real numbers, $\Rb$, we can
apply the property $\sup(\emptyset)=-\infty$
\cite{cantrell:extendedreal}; note that here $\emptyset$ means an
empty set of $\Rb$. %
Let $m=(-\infty,\infty)$ be a measure with infinite time and value.
We define the \emph{supremum} of $S$, noted as $\sup S$, as
\[
\sup S =\begin{cases}
...
\end{cases}
\]
where $T(m)=-\infty$ is a consequence from the extended real numbers
property and $V(m)$ could be any value but we generalise it
...
sets. The traditional set union operations do not apply to time series because
the result time series may have repeated time values.  Thus, we 
...
%
The \emph{temporal union} of $R$ and $S$, noted $R \cupt S$, is a
...
\begin{example}\label{ex:model:s1s2}
  ...
  contains measures that share both time and value attributes, as it
  is measure $(4,0)$. The central left area contains the measures in
  $R$ that only share the time attribute with a measure in $S$, as it
  is measure $(3,1)$. The central right area has a symmetrical
...
Time series \emph{difference} can also be defined. Similar to union, the
...
into a single value.  It is worth noting that it is a special
case of fold.
...
A time series can be thought as a discrete representation of an
(original) temporal function. In this section we devise some
operations that manage time series according to this temporal
function standpoint.  
%
...
From temporal interval other operators can also be defined such as temporal
selection, temporal concatenation, or temporal join. As an example, the
definition of temporal selection operation is given.
...
\cup S[t_1:t_1]^r \cup \dotsb \cup S[t_k:t_k]^r$. If $t$ is a time
instant, note that the temporal selection depends on the temporal interval
...
The temporal selection operation also allows to regularise an irregular
...
\emph{time series} as defined in Section~\ref{sec:model:TSMS}.  A
\acro{MTSMS} stores a time series as a structure that we call
\emph{multiresolution time series}.
%
...
name \emph{consolidation}  this action of regularisation.  Note that
...
assume that there are no repeated $(\delta,f)$ pairs in $M$.
...
queries can be applied to an \acro{MTSMS} by using \acro{TSMS}
operations. For example, let $L$ and $M$ be two multiresolution time
series with the same multiresolution parameters. We can compute the
sum of both as $\totalseries(L) + \totalseries(M)$. Recall that $R+S$
is the sum of the values of two time series $R$ and $S$, which has
been defined in Example~\ref{ex:model:sum} as a binary computational
operator.
...
summarise a time series, for example it is possible to calculate a
statistical indicator of the time series such as the average or a more
complex digital signal processing operation as proposed
in~\cite{zhang11}. Furthermore, the representation of a time series
...
%
However, in some cases it is preferable for $T(m)$  not to match the
...
closed interval $S'=S[\tau:\tau+\delta]$, or other combinations as it is
...
The time series representation in the previous examples can be
...
\begin{example}\label{ex:model:smultiresolution} 
  ...
  $\tau_1=20$, so let $M_{29}$ be the multiresolution time series
  at this state.
  ...
  twice the resolution of $R_1$ and $k_0$ is bigger than $k_1$. 
\end{example}
