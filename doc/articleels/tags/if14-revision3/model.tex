...
Intuitively speaking, a \emph{time series} is an ordered set of
...
manage time series according to this point of view. Then, these
temporal function operations may return new measures that are not
contained in the time series.

Let $S\subset\cal{T}\times\cal{V}$ be a time series.  A \emph{temporal
...
implement, therefore for every representation a particular temporal
interval must be interpreted:
...
and $m' = f(S,\tau,\delta)$. This resulting measure summarises the
data of $S$ comprised in the given interval.  After the consolidation,
we can remove from the buffer the consolidated part of the time
series. Therefore, we discard historical data.
...
$(\delta,f)$ parameters among a multiresolution series, thus they act as
key attributes.
...
$\delta_{i_0} < \delta_{i_1} < \cdots < \delta_{i_k}$, being
$\delta_i$ the resolution step of the resolution subseries $R_i$.
Recall that $R||S$ is the concatenation of two time series $R$ and
$S$, which we defined in Section~\ref{sec:sequence}.
...
queries for a \acro{MTSMS} by using \acro{TSMS} operations. For
...
representation method. In this regard, we can consider some
...
\item The $mean^\dd$ is such that $V(m) = \frac{1}{\delta}
  \sum\limits_{\forall n \in S'} V(n)$. Note that for the Dirac delta
  function $\int\dd(t)dt=1$. Note that $\sum\limits_{\forall n \in S'}
  V(n)$ is a sum of values that could be implemented as
  $\agg(S',(0,0),f)$ where $f(m,n)=(0,V(m)+V(n))$, as shown in
...
\item The $\meanz$ is such that $V(m) = \frac{1}{\delta}
  \big[(T(o)-\tau)V(o) + \sum\limits_{\forall n\in R}( T(n)- T(\prev_S
  n) )V(n)\big]$ where $o=\min S'$ and $R= S' - \{o\}$. Note that
  $\meanz$ is a sum of values that could be implemented as $\meanz =
  \frac{1}{\delta}\agg(S',(0,0),f)$ where $f(m,n)=(0,V(m)+v)$ and
\[
 v=
 \begin{cases}
...
\begin{example}\label{ex:model:smultiresolution}
  ...
  it can not be consolidated further. As $T(\max S)=29$, the last
...
enhancements.  We exemplify what happens when we add a new measure
