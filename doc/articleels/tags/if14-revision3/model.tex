...
a function $f$ defined as $f:\mathcal{P}(\mathcal{T}\times\mathcal{V})
\times \cal{T}\rightarrow\cal{V}$ such that for every time instant
$t\in \cal{T}$, $\exists v \in \cal{V}: v=f(S,t)$.
There is not an unique way to obtain $f$ for a given time series
$S$. Because of this, we will use another notation for temporal
representation functions $S(t)^r = f(S,t)$ where $r$ is a superscript
that shows the name $r$ of the representation method used.  In some
sense, we can think of $S(t)^r$ as the original temporal function from
which $S$ was obtained.  Then, $S(t)^r$ means the representation
function of $S$ using method $r$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Below, we exemplify the representation functions
