...
This paper introduces a model for \acro{TSMS} that stores and manages
time series data.
...
%
First, in Section~\ref{sec:model:TSMS} we introduce a \acro{TSMS}
model that is devoted to time series basic elements and operations.
%
Second, in Section~\ref{sec:MTSMS} we introduce a \acro{MTSMS} model
...
the \acro{TSMS}+\acro{MTSMS} model.  Section~\ref{sec:example} is
...
% A \acro{MTSMS} adds multiresolutio ch a \acro{TSMS} with multiresolution
...
Multiresolution enhances \acro{TSMS} features in several aspects:
...
\item Data validation. Monitoring systems capture data but are the
  origins of some drawbacks that will affect later the process of time
  series. The main problems arise when monitors cannot capture data,
  producing errors known as gaps, or capture data erroneously,
  producing outlayers~\cite{quevedo10}.  The multiresolution
  attribute functions are designed to cope well with validating,
  filtering and reconstructing with these unknown data in order to
...
\item Data time regularising. Another monitoring side effect happens
  ...
  data are not equi-time spaced. These irregularities can come from
