...
multiresolution. In this section we motivate the advantages of
this approach. First, we introduce the concept of multiresolution
through a example. Then, we discuss the advantatges of this approach.
...
%
In this example, aggregations function corresponds to a statistical
mean. The before \emph{init} are not acquired and it is marked as
unknown (\emph{u}). Future values are also marked as unknown \emph{u}
until time advances. Therefore, the first summary at \emph{now} time
for the original signal corresponds to values $\{u,u,2,5,8,u\}$.
