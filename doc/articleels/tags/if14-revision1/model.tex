...
attribute aggregation policies. Disks are size bounded so they only
...
\todo[color=yellow]{Decidir si regularitzar o consolidar. S'usen tots
  dos indistintament}
...
subserie $R_i$. Then, $\totalseries(M) = S_{i_0} || S_{i_1} || \cdots
|| S_{i_k}$.
%
...
$m=\min(S[\tau,\tau+\delta))$, then the resulting time fullfils that
$\tau\leq T(m) < \tau+\delta$.
...
That is, the time series aggregated is interpeted by the temporal
...
schemas differ basically on the interval selection operation. However,
