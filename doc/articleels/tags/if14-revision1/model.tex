...
order, especially when $f^f$ is not commutative.  Then, it is possible
...
by value.  We define the map function $f^m: (t,v)\mapsto (t,t\times
v)$. Then $\map(S,f^m)=\{(1,1),(2,6),(4,4)\}$.


Aggregate operator allows computing a measure that results from the
sum of all values.  We define the aggregate function $f^a: (t_a,v_a)
\times (t_b,v_b) \mapsto (0,v_a+v_b)$. Then $\agg(S,(0,0),f^a) =
...
equals one.  We define the fold function $f^f: S_a \times (t_b,v_b)
\mapsto S_a \cup S_b$ where $S_b=\{(t_b,v_b)\}$ if $v_b=1$ or
$S_b=\{\}$ otherwise. Then $\fold(S,\{\},f^f)=\{(1,1),(4,1)\}$.
...
temporal range, $S[t_i,t_f]^r\equiv S(t)^r$ for all $t \in [t_i,t_f]$. This
