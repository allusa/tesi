...
Let $m_i$ and $n$ be two measures. The \textbf{XXXXX} \todo{tuple
  order?} between $m$ and $n$ is the binary relation written $m\leq n$
and defined as $m\leq n\iff (T(m)<T(n) \vee m = n)$. It could be
proved that this is a partial order.
...
possible order relations: temporal and partial\todo{canvi de
  nom}. Consequently, these two possible order relations of measures
impose two operation definitions for some set operators.  Mainly they
impose two membership operations and so to all operations based on it.


The \emph{membership} defines when one measure belongs to a time
series. Considering the partial\todo{nom} order of measures, the set
