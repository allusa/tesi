...
$m_i = (t_i, v_i)$ and $m_j = (t_j, v_j)$ be two measures.  (i) A
\emph{partial order} is obtained when both measures are equal as a
whole, $m_i \geq m_j$ if and only if $t_i > t_j$ or $(t_i, v_i) = (t_j,
v_j)$, as then two measures are not ordered when only $t_i =
t_j$. (ii) A \emph{total order} is obtained when both measures are
equal only considering the time attribute, $m_i\geq m_j$ if and only if
$t_i\geq t_j$, as then measures are always ordered.  We also call
...
repeated at $S_2$: $S_1 \cup S_2 = \{m_1 \in S_1 \vee m_2 \in S_2 |
...
\{ m_1 \in S_1 \vee m_2 \in S_2 | m_1 \notinst S_2 \vee m_1 \in S_2,
m_2 \notinst S_1 \}$.
...
time series and $\fmap$ a function over a measure where $\fmap: m \mapsto
m'$. The \emph{map} of $\fmap$ to $S$ is a time series $\map(S,\fmap) =
\{\forall m_i \in S : \fmap(m_i) \}$.

Fold recursively combines every measures of a time series.  Let
$S_1=\{m_0, \dotsc, m_k\}$ and $S_2$ be two time series, and $\ffold$ a
function over a time series $S$ and a measure $m$ resulting in a
time series $S'$, $\ffold: S \times m \mapsto S'$. The \emph{fold}
of $S_1$ by $\ffold$ with initial value $S_2$ is a time series
$\fold(S_1,S_2,\ffold) = \ffold(\dots( \ffold(\ffold(\ffold(S_2,m_0),\allowbreak
...
compute aggregate statistics.  Let $S=\{m_0, \dotsc, m_k\}$ be a time
series, $m$ a measure, and $\fagg$ a function over two measures where
$\fagg: m_i \times m_j \mapsto m'$. The \emph{aggregate} of $S$ by
$\fagg$ with initial value $m$ is a time series $\agg(S,m,\fagg) =
\fagg(\dots( \fagg(\fagg(\fagg(m,m_0),\allowbreak m_1),\allowbreak
m_2 )\dots),\allowbreak m_k)$.
...
\item Let $S(t)^\dd$ be the \dd{} representation for $S$. The
  \emph{\dd{} temporal interval} is $S[t_i,t_j]^\delta = S[t_i,t_j]
...
defined in Section~\ref{sec:model:operations} and the remaining
operations could be implemented with aggregation functions. As
instance, $\sum\limits_{\forall m \in S'} V(m)$ is a sum of values
that could be implemented as $\agg(S',(0,0),f^a)$ where $f^a:
