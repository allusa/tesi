...
\begin{definition}
  Let $v$ be a value and let $t$ the time instant when the value has
  been acquired. A \emph{measure} $m$ is defined as the tuple
  $m=(t,v)$.
\end{definition}
...
Let $m$ and $n$ be two measures. The \textbf{XXXXX} between $m$ and $n$ is
the binary relation noted $m\leq n$ and defined as $m\leq n\iff
(T(m)<T(n) \vee m = n)$. It could be proved that this is a partial
order.
...
every measure.  Let $S$ be a time series and let $f:m \mapsto m'$ a
