...
\sup S =\begin{cases}
  ...
  -\infty   & \text{otherwise}
\end{cases}
\]\todo{ha de retornar mesura (-inf,inf)}
...
%
\todo{te sentit en la unio el concepte 'temporal'? te el mateix sentit
  que a la pertanyensa? No aplicaria millor el concepte 'unio
  simetrica'?}
% mentre que a les operacions següents podem trobar una relació amb la pertinença, a la unió no li trobo. Per coherència amb les altres també definia unió i unió temporal tot i que de forma arbitrària. Potser sí que seria millor dir-li simètrica.
%
...
%
\todo[color=yellow]{No se si els papers d'R i S no estan
  intercanviats. No es R la serie transformada i S els valors
  inicials?}
% No, seguint el cas simple següent d'agregació es veu més clar el
% paper de cada paràmetre
...
\todo[color=yellow]{ D'aqui fins el seguent apartat el plantejament no
  m'entusiasma. Em sembla que és massa complicat. El concepte
  d'interval temporal no descriu gaire el que es i la idea de
  sellecion temporal no respon a una seleccio sino mes aviat a un
  resampling.

  Tal i com ho veig, d'uns serie en derivem (mes d'una) funcio
  original ---tampoc no veig per que en diem de representacio ja que
  mes aviar es al reves: la serie vol ser una representacio de la
  funcio original. Amb aquesta funcio original ens podem replantejar
  fer resamplings, i.e. avaluar-la en certs instants de temps. Aquests
  resamplings son noves series temporals que tenen certa relacio amb
  la serie inicial.

  Com veus aquest plantejament? que em perdo?
}

% Més aviat una sèrie temporal representa uns valors adquirits en uns
% instants de temps (una funció original discreta f(k)). Aquest
% problema podria provenir de l'adquisició d'una variable contínua
% (una funció original contínua f(t)). Aleshores plantegem S(t)
% equivalent a f(t). També podriem plantejar S(t) equivalents a f(k)
% però aquí no ho fem, serien funcions parcials. A partir de S(t)
% clarament podem dibuixar el gràfic de la sèrie temporal, descrit com
% a graf més general (p.ex. quan no el podem dibuixar). Per tant,
% podem dir que S(t) té molta relació amb la representació gràfica de
% la sèrie temporal?
%
% Potser millor dir
% que la sèrie temporal és una modelització pel problema de la funció
% original?
%
% Interval i selecció: potser sí que els noms no són els
% adequats. Resampling també estaria bé. L'interval es diu així perquè
% té la forma S[x,y] tot i que particularitzat per r; és una
% interpretació en matemàtica discreta de S(t) en un interval [x,y] i
% en el context de r.
...
% \todo{Aixo moure-ho al final de l'apartat anterior}
% %
...
\acro{MTSMS} for a single multiresolution time series. A
multiresolution time series is a collection of resolution subseries
%
\todo[color=yellow]{El terme resolution susbserie es confus. El que es
  una subserie ja s'ha dit abans i te a veure amb un tipus de
  dades. D'altra banda el concepte resolution subserie mes aviat fa
  referencia a una peça de l'aruitectura (penso).}
%
which temporarily accumulate measures in a buffer in order to select
some data and finally store them in a disc.
%
\todo[color=yellow]{Aqui el terme disc es confus. ja ho hem comentat
  molts cops pero ara pren mes rellevancia. Disc es un terme que pel
  lector significa una cosa clara i aqui confon. Bounded storage? Circular storage? }
%
The data selection process
%
\todo[color=yellow]{Que es aquest process?. No s'explica abans.}
%
...
Regarding operations, the \acro{MTSMS} model needs operators to change
...
% La entradeta necessita un remake: despres de refer l'apartat
...
% No cal explicitar interval
% Each $\tau_n$ determines a time interval $I_n=[\tau_n,\tau_n+\delta]$. 
...
%
\todo[color=yellow]{Per que $f$ no es defineix directament sobre una serie i
  prou?. Llavors es podria expressar la consolidacio com
  $f(S[\tau_n,\tau_n+\delta])$. No fora millor?}
...
\geq \tau+\delta$.
%
\todo[color=yellow]{Aqui la cosa no quadra. No cal afegir en ordre per
  que una serie es una estructura desordenada. En tot cas per
  consolidar cal que les mesures de l'interval hi siguin.}
...
There are two basic time series queries for a \acro{MTSMS}: (i)
...
interval. For example $S' = S[x,v]$. Second, a \acro{TSMS} aggregation
