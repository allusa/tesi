...
%
\todo[color=yellow]{Afegir citacio}
%
...
\todo[color=green]{Dominis en lletra caligrafica}
...
\begin{definition}[Time series]
  ...
  We define the domain of a time series $S$, denoted as $\dom(S)$ \todo{sense parentesis?}, as
...
$\dom(S)={\cal{V}}_1\times\cdots\times{\cal{V}}_n$\todo{$\cal{T}$
  també?}. Then, we write its measures as $m=(t,v_1,v_2,\ldots,v_n)$.
...
%
\todo[color=green]{Compte amb la notacio per definir funcions. Veure
  la wikipedia anglesa (apartat corresponent)}.
...
$\map(S,f)=\{(1,1),(2,6),(4,4)\}$.  
%
\todo[color=green]{times es pel producte cartesia}
...
\todo{inf i sup retornen mesures, cal indicar T(inf)}
...
% \todo{Aixo ja s'ha dit. Potser moure la definicio formal com a exemple
%   en algun lloc per no perdre-la}
% %
