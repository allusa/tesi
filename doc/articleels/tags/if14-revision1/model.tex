...
membership operation applies. 
%
Let $S$ be a time series and $m$ be a measure. The \emph{membership}
is denoted $m \in S$. Considering the temporal order of measures, we
define the \emph{temporal membership} as $m \inst S$ when $\exists m_i
\in S : T(m) = T(m_i)$.  Let $S_1$ and $S_2$ be two time series. From
membership operations we could define \emph{inclusion} $S_1\subseteq
S_2$ and \emph{temporal inclusion} $S_1\subseteqt S_2$.
