...
\item L'interval no és múltiple dels intervals de multiresolució
  ...
  assenyala una possible mesura en l'instant $t_b$.

  Cal tenir en compte que $f_0(S',[t_a,t_b])$ ha de resoldre la
  selecció en l'interval: a) amb una selecció d'interval
  $S'[t_a,t_b]=\{ (\tau_0+\delta, f'(v_{i_0},\dotsc,v_{i_1}) ) \}$, b)
  amb una selecció d'interval temporal $S'[t_a,t_b]^r=(\tau_0+\delta,
  f'(v_{i_0},\dotsc,v_{i_1}), (t_b, f^r(f'(v_{i_1+1},\dotsc,v_{b}
  ,\dotsc,v_{i_2}))) ) $ on $f^r$ és la interpolació realitzada per la
  funció de representació $r$, c) amb altres casos, podem pensar per
  exemple $S'[t_a,t_b]^r=(\tau_0+\delta, f'(v_{i_0},\dotsc,v_{i_1}),
  (t_b, f'(v_{i_1+1},\dotsc,v_{b} ,\dotsc,v_{i_2})) )$.


  Així, per a estudiar $d(r_1,r_2)$ s'ha d'analitzar
  $r_1=f'(v_{i_0},\dotsc,v_{i_1},\dotsc, v_{i_1+1},\dotsc,v_{b})$ i,
  depenent de la selecció, a) $r_2=f'(f'(v_{i_0},\dotsc,v_{i_1}))$, b)
  $r_2=f'(f'(v_{i_0},\dotsc,v_{i_1}),f^r(f'(v_{i_1+1},\dotsc,v_{b},\dotsc,v_{i_2})))$,
  o c)
  $r_2=f'(f'(v_{i_0},\dotsc,v_{i_1}),f'(v_{i_1+1},\dotsc,v_{b},\dotsc,v_{i_2}))$.
...
\todo{} Podem intentar afitar l'error en el cas dels intervals no
múltiples?  En resum el problema és: caldria calcular $f'(v_{i_1+1},\dotsc,v_{b})$
però la informació que hi ha emmagatzemada per a aquest interval és
$f'(v_{i_1+1},\dotsc,v_{b},\dotsc,v_{i_2})$.

Cas del màxim: $\max(v_{i_1+1},\dotsc,v_{b})$ i
$\max(v_{i_1+1},\dotsc,v_{b},\dotsc,v_{i_2})$; per tant si el màxim no
es troba a $[t_a+\delta,t_b]$ aleshores no es pot afitar l'error
perquè $\max(\dotsc,v_{b},\dotsc,v_{i_2})$ ves a saber.

$\epsilon=d(r_1,r_2)= \max(v_{i_1+1},\dotsc,v_{b},\dotsc,v_{i_2}) -
\max(v_{i_1+1},\dotsc,v_{b})$ que no és fitable.


Cas del total: $\sum(v_{i_1+1},\dotsc,v_{b})$ i
$\sum(v_{i_1+1},\dotsc,v_{b},\dotsc,v_{i_2})$.  Aleshores
$\epsilon=d(r_1,r_2)= \sum(v_{i_1+1},\dotsc,v_{b},\dotsc,v_{i_2}) -
\sum(v_{i_1+1},\dotsc,v_{b}) = \sum(v_{b+1},\dotsc,v_{i_2}$.  En
aquest cas tampoc és fitable. Però si la variable mesurada fos
monòtona creixent, per exemple un comptador monòton creixent,
aleshores $0\leq \sum(v_{i_1+1},\dotsc,v_{b}) \leq
\sum(v_{i_1+1},\dotsc,v_{b},\dotsc,v_{i_2})$.  I per tant com a màxim
es cometria un error de $\epsilon=d(r_1,r_2)\leq
\sum(v_{i_1+1},\dotsc,v_{b},\dotsc,v_{i_2})$; és a dir del valor
consolidat a $\tau_0+2\delta$ que significaria que en els pitjors del
casos tota la mesura hauria ocorregut després de $t_b$.
...
\begin{example}[Aplicació de la conservació de la mitjana de la funció
  ...
  comptador la mesura de l'energia és exacte, dins de la precisió del
  comptador, a diferència d'un aparell que mesuri potència instantània
  del qual només es pot deduir l'energia exacta si es considera que el
  senyal es pot reconstruir: per exemple compleix Nyquist tot i que a
  la pràctica és complicat conèixer la freqüència de les variables
  mesurades atès que solen ser aleatòries. A la inversa, a partir de
  la mesura de l'energia només es pot deduir la potència instantània
  exacta si es considera que el senyal es pot reconstruir.

  En resum, sigui $E(t)$ l'energia d'un senyal i $P(t)$ la potència
  instantània del senyal, es compleix la relació $E(t)=\int P(t)
  dt$. Sigui $[t_a,t_b]$ un interval de temps, un comptador mesura
  exactament el valor de $\int_{t_a}^{t_b} P(t)$. En canvi, un aparell
  de mesura de potència instantània es capaç de mesurar exactament
  $P(t_a)$ i $P(t_b)$. Ara bé, a partir del comptador no es poden deduir
  exactament $P(t_a)$ i $P(t_b)$ i a partir de la mesura de la
  potència instantània no es poden deduir exactament $\int_{t_a}^{t_b}
  P(t)$. Tampoc a partir del comptador es poden deduir exactament
  energies que no s'han mesurat, per exemple ni
  $\int_{(t_a+t_b)/2}^{t_b} P(t)$ ni $\int_{(t_a+t_b)/2}^{t_c} P(t)$
  on sigui $[t_b,t_c]$ un interval de temps mesurat; ara bé sí que
  serà exacte el càlcul $\int_{t_a}^{t_b} P(t)+\int_{t_b}^{t_c} P(t)$.
  
  No totes les variables físiques són susceptibles de
  ser mesurades amb un comptador. Els comptadors es poden aplicar per
  exemple per a mesurar energia elèctrica, persones que entren en un
  edifici, etc. 
  

  La multiresolució és capaç de conservar aquesta exactitud del
  comptatge total. 
  El comptatge total es pot conservar amb una multiresolució amb
  funcions d'agregació per atributs de suma o bé per atributs de
  mitjana de la funció. Aquests darrers són els que permeten a més
  expressar la sèrie temporal resultant de la multiresolució de forma
  més coherent amb l'original (vegeu secció de naturalesa de comptadors \todo{ref}).
  ...
  \todo{falten algunes imatges d'acompanyament}
...
\todo{}
* analitzar casos on és el mateix aplicar un agregador que un altre: per exemple la mitjana i l'àrea quan la sèrie temporal és regular



% \subsubsection{Exemple2}


% Quan una sèrie temporal és regular, l'interpolador mitjana i l'àrea valen el mat
% eix en l'interval $[T_0,n\delta]$.
