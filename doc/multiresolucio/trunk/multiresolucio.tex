...
\begin{example}[Mateixa consulta i funció d'agregació d'atributs per a
  ...
  $o(S)=V(f(S,[-\infty,\infty]))$. Analitzem els efectes segons dues
...
\todo{és més simple fer el cas amb la suma que amb la mitjana de la funció}
...
\begin{example}[Consultes amb subresolucions]
\label{ex:multireoslucio:informacio-subresolucions}
\todo{}

Ara o no s'aplica a $S[-infty,infty]$ sinó a $S[t_a,t_b]$. Ens quins
casos és cert?  De la multiresolució jo sé que és cert en els
intervals consolidats $[i_0,i_1]$. Per tant si $t_a,t_b$ és múltiple
dels intervals consolidats perfecte. Sinó cometo error. En el cas de
totals el puc quantificar: per exemple si $t_a=i_0$ i $t_b < i_1$,
aleshores l'error màxim que puc cometre és el que hi ha mesurat a
l'interval $[i_0,i_1]$; és a dir que en els pitjors del casos tota la
mesura hauria ocorregut després de $t_b$. En el cas del màxim, ves a saber.
...
* la mitjana sí que és associativa en el cas que la sèrie temporal sigui regular
 : no n'hi ha prou amb ser regular, s'ha de complir quelcom més per exemple que $\delta$ sigui múltiple del període de temps regular. En realitat volem que es compleixi $|S[\tau_0,\tau_0+\delta_0]|=|S[\tau_0+\delta_0,\tau_0+2\delta_0]|=\dotsb = |S[\tau_0+(M-1)\delta_0,\tau_0+M\delta_0]|$
...
\begin{example}[Aplicació de la conservació de la mitjana de la funció
  ...
  l'\autoref{ex:multireoslucio:informacio-subresolucions} de
  subresolucions\todo{nom?}, en consultes en què el període no es
  correspongui amb les resolucions emmagatzemades, els totals no seran
  els correctes que s'obtindrien de calcular amb les dades originals.
  De fet, és el mateix problema que hem exposat que a partir d'un
  comptador no es poden deduir exactament energies que no s'han
  mesurat.
