...
\todo{falta percebre que al SGST s'hi emmagatzema la sèrie temporal original}
\todo{enfocar-ho diferent: emmagatzem la S als dos sistemes. 1. A cadascun hi podem fer les consultes pertinents  2. S'hi seguim certes restriccions obtenim el mateix resultat en el SGST que en el SGSTM}
...
important.  
\todo{dir que després a les aplicacions solucionarem
...
%  més enllà del temps de càlcul
% de cada funció d'agregació: aquest limita quantes sèries temporals
% multiresolució diferents i quantes resolucions de cada es poden
% emmagatzemar un mateix aparell.
...
Així doncs, el sistema dual es pot utilitzar en les aplicacions següents:
\begin{itemize}
\item 
...
O també pot servir per si no es compleix l'ordre d'arribada de les mesures: aleshores el SGST i el SGSTM no donarien la mateixa informació, per tant es pot obligar a refer la informació que hi ha desada el el SGSTM


\item Farciment inicial del SGSTM: imaginem que tenim dades i volem passar a emmagatzemar-les en un SGSTM (això sobretot va lligat a la implementació en para\l.lel \todo{}
...
\item Quan es modifiquen les dades, s'aplica la mateixa operació a la
  ...
  la relació entre $\text{mod}$ i $\text{mod}'$.  En un context on
  només es permeten operacions d'afegir però no de modificar, és el
...
En resum, la sèrie temporal resultant dels sistemes duals pot ser considerada 

\todo{lligam de que la S' pot ser considerada una vista}
% If we regard the resulting
