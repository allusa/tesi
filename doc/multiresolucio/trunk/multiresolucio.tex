...
La compressió de dades es formalitza en la teoria de la informació
(\emph{information theory}). Per al cas particular de compressió amb
pèrdua s'aplica un subconjunt de la teoria anomenat teoria de la taxa
de bit-distorsió %optimot: rate-distortion relationship
...
\subsection{Informació de la multiresolució}

El model dels \gls{SGSTM} es basa en una compressió
amb pèrdua; és a dir descartar dades i emmagatzemar només aquella
informació que es consideri necessària o suficient.
...
de multiresolució $d(r_1,r_2)$ on $d$ és una funció que permet avaluar
...
problema que intenten resoldre els \gls{SGSTM}.  Els \gls{SGSTM}
resolen un problema de compressió amb pèrdua (NOO perquè l'aproximació és una pèrdua)
 i en canvi aquest seria
un problema d'aproximació al senyal original, el qual altres models
poden resoldre més bé\todo{ref?}.
...
Cal pensar amb consultes també qualitatives, 
per exemple la consulta  podria ser o=Creix la sèrie temporal? Si la resposta és Sí en els dos casos, aleshores no hi ha error.

També hi pot haver casos on l'operació per a obtenir el mateix resultat és diferent en els dos casos; és a dir $r_1=o(S)$ però  $r_2=o'(S')$ on $o'$ és l'operació equivalent a $o$ per aplicar després de la multiresolució. Per exemple, una operació $o$ que es vulgui fer pot ser el càlcul de la multiresolució, $r_1=\glssymbol{not:sgstm:multiresolucio}(S,e)$, aleshores l'operació $o'$ equivalent és la identitat perquè el \gls{SGSTM} ja ha calculat la multiresolució,  $r_2=S'$ on $o'$ no hi és. Aquest exemple, de fet, és el cas de la funció de multiresolució que ja hem discutit abans on és equivalent l'operació de multiresolució aplicada als SGST que la sèrie temporal dels SGSTM; per tant l'error  $d(r_1,r_2)$ seria nul.
...
* Si el que es vol inicialment ja és fer el càlcul de multiresolució aleshores un SGSTM ja ho fa exactament, tal com s'ha discutit en l'equivalència de multiresolució i en el cas de SGST duals.


* Si el que es vol és un emmagatzematge reduït aleshores cal avaluar la pèrdua d'informació.

  - Comprovar que la visualització de la informació original amb les comprimides s'assembli: aquest cas tindria una valoració subjectiva de la mateixa manera que en la compressió amb pèrdua de multimèdia. Per exemple el criteri de RRDtool es basa en l'amplada de la pantalla. Això pot ser té relació amb el camp de gràfics 3D?

  - Comprovar que la resposta a una consulta és la mateixa en les dades originals que en les dades en el SGSTM.

  - Quines consultes no podré respondre en un SGSTM

  - Consultes que es poden resoldre a la perfecció, per exemple en el cas de comptadors
...
% \subsubsection{Multiresolució sense pèrdua}
