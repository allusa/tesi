...
\begin{tikzpicture}[thick,scale=0.8, every node/.style={transform shape}]
 \tikzset{
  ...
  node[above right] {Multiresolution time series};


  %discmig
  \node [above=0.25cm of m.center] (discr1) {...};

  %discr

  \node [ellipse,draw,minimum height=3.5cm,minimum width=2.5cm,alias=discr0] [left=of discr1] {};
  \node[below=0cm of discr0] (rs0) {resolution subseries};
  \node[below=0cm of rs0] {$R_0$};
  ...
  \node[below=0cm of discrd] (rsd) {resolution subseries};
  \node[below=0cm of rsd] {$R_d$};
  ...
  %mesura 
  \node[above=1cm of m.north] (m0) {Time series};

  \draw[myarrow] (m0) -- (m.north) 
  node[right,midway] {measure};
