...
Time series are usually stored and managed by SQL relational database
management systems. However, using SQL systems as a time series
backend suffers some drawbacks
...
\cite{atzeni13:relational_model_dead,stonebraker10}.  When having huge
amount of data, time series data mining proposes to apply time series
analysis methods to compute the queries. One approach is to find the
optimal solution approximation versus cost to the original signal and
then compute analysis such as similarity or pattern search
\cite{fu11,keogh01,last01}.  On another approach, RRDtool
...
visualisations. However, RRDtool is very specific to the computer
network field such as applications of network counters.


\todo{}
In more general approaches, aggregates for massive data streams are studied \todo{see graham}. However, treating time series as data streams does not consider adequately the time dimension nor computes the evolution of aggregated parameters along time, which is interesting for monitoring purposes. 
...
two separated submodels, one a \acro{TSMS} model mainly for describing time
series and the other a \acro{MTSMS} model for describing multiresolution over
time series.
...
describes \acro{TSMS} and \acro{MTSMS} with information systems
theory.  We propose multiresolution as a lossy storage solution that
selects only the needed information. The concept is similar to
multimedia lossy compression methods, where information can be
discarded in favour of size, but applied to time series.
...
\todo{treball del Graham Cormode}
Parla de sketch, que són algoritmes per calcular mitjançan orientació a flux (stream). Això es pot aplicar al model de MTSMS: les mesures s'insereixen en ordre de temps, la $S_B$ esdevé de cardinal 1 i les $f$ queden restringides a aquelles que es poden calcular en flux. De fet, és el patró que segueix RRDtool.

Paraula Clau que apareix: Aproximate Query Processing (AQP)
One possible avenue of
attack is via approximate query processing techniques over
probabilistic data. As in conventional database systems, such
techniques need to rely on effective data reduction methods
that can effectively compress large amounts of data down
to concise data synopses while retaining the key statistical
traits of the original data collection [6]. It is then feasible to
run more expensive algorithms over the much compressed
representation, and obtain a fast and accurate answer. In
addition to enabling fast approximate query answers, such
compact synopses can also provide the foundation for
human understanding and interactive data exploration, and
probabilistic query planning and optimization.
