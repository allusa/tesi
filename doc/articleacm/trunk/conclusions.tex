...
\todo{relacionar-ho amb el nostre model}
...
\todo{Some time concepts may be taken into account by TSMS, such as the discussions about time granularities }
...
\todo{Our model represents a generic approach to the multiresolution features, we define it very open so that any attribute aggregate function can be defined}
...
\todo{These techniches of compression are interesting for being used as attribute aggregate functions in the MTSMS model}
...
\todo{Our TSMS model proposes time series as more firmly based on relational algebra, clarifying this ambiguity}
...
performance.
\todo{MTSMS model also can be thought as an stream processor, RRDtool is very stream oriented}
\todo{dir que si seguim l'addició amb ordre de les mesures, aleshores es pot fer l'stream en els MTSMS ja que no hi ha possibilitat d'operacions d'UPDATE}
...
\todo{Comentari de future work sobre avaluar la qualitat de les mesures}
