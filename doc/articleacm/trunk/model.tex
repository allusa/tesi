...
\todo{comentari sobre els operadors i que la semàntica l'ha de donar qui aplicqui l'operador}
...
The architecture of MTSMS model is shown in
...
Attribute aggregate functions \todo{dir que són un cas especial dels agregadors de TSMS} summarise time series information
...
\begin{example} Queries over a defined multiresolution schema for an
...
\]
%alerta: si hauria de contenir un temps que a dmap resultant no hi ha de ser per tal de calcular prev_{S_I}(mi) 
...
\]
% potser millor \multiresolution(S,schema) on schema es Sd
...
$S'=\totalseries(M)$ to \acro{MTSMS} and $S'= \multiresolution(S)$ to
\acro{TSMS} is plotted, however the implementation consideration are
